...
export default function JobDetailSlideOver() {
  ...
  const [isEditMode, setIsEditMode] = useState(!jobId); // Auto-edit mode for new jobs
  const [isSaving, setIsSaving] = useState(false);
  const [error, setError] = useState(null);
  
  if (loading) return <LoadingSpinner fullScreen={true} size="md" />;
  
  const job = jobId ? jobs.find(j => j.id.toString() === jobId) : null;
  const isCreateMode = !jobId;
  const open = !!jobId || isCreateMode;
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      if (isCreateMode) {
        // CREATE new job - using your POST route
        ...
        // UPDATE existing job - using your existing PATCH route
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `Failed to ${isCreateMode ? 'create' : 'update'} job: ${response.statusText}`);
      }
      
      await refetch();
      
      const successMessage = isCreateMode 
        ? `Job "${formData.title}" created successfully!`
        : `Job "${formData.title || job.title}" updated successfully!`;
      
      toast.success(successMessage);
      
      if (isCreateMode) {
        // Close slideover after successful creation
        navigate('/app/jobs');
      } else {
        setIsEditMode(false);
      }
      ...
      const errorMessage = err.message || `An unknown error occurred during ${isCreateMode ? 'creation' : 'update'}.`;
      setError(errorMessage);
      toast.error(errorMessage);
      console.error(`Error ${isCreateMode ? 'creating' : 'updating'} job:`, err);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleCancelEdit = () => {
    if (isCreateMode) {
      // Close slideover if canceling creation
      navigate('/app/jobs');
    } else {
      setIsEditMode(false);
      setError(null);
  ...
  // For new jobs, create empty job structure
  ...
  }[currentJob.priority] || "bg-gray-500/20 text-gray-300 border border-gray-500/50";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center space-x-3">
          <h2 className="text-3xl font-extrabold tracking-tight">
            {isCreateMode ? "Create New Job" : (isEditMode ? "Edit Job" : "Job Details")}
          </h2>
        </div>
      ...
      {/* Content - Always show Edit Form for create mode, switch for edit mode */}
      {(isEditMode || isCreateMode) ? (
        <JobEditForm
          job={currentJob}
          onSave={handleSave}
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 border-b border-spotify-light-gray/10 pb-4">
            ...
            <span className={`flex-shrink-0 px-3 py-1 text-xs rounded-full font-bold uppercase tracking-wider ${priorityColor}`}>
              {currentJob.priority?.charAt(0).toUpperCase() + currentJob.priority?.slice(1) || "Unknown"}
            </span>
          </div>
          {/* Tags */}
          ...
          <div className="border-b border-spotify-light-gray/10 mb-8 pb-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-4 gap-x-6">
              <DetailItem title="ID" value={currentJob.id} />
              <DetailItem title="Status" value={currentJob.status} />
              <DetailItem title="Department" value={currentJob.department} />
              <DetailItem title="Employment" value={currentJob.employmentType} />
              <DetailItem title="Experience" value={currentJob.experience} />
              <DetailItem title="Openings" value={currentJob.openings} />
              <DetailItem title="Location" value={currentJob.location} />
              <DetailItem title="Salary" value={currentJob.salary} />
              <DetailItem title="Fill By" value={formatDate(currentJob.fillBy)} />
              <DetailItem title="Created At" value={formatDate(currentJob.createdAt)} />
            </div>
          </div>
          {/* Job Description */}
